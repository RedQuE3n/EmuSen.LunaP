...
namespace EmuSen.LunaP.Controls
{
    ...
    public class LunaTable<T> : LunaTable where T : class
    {
        ...
        // columns cost nothing to draw whole, and the machinery below would spend a range
        // computation per layout to leave every one of them exactly where it was.
        //
        ...
        //
        // WHAT IT COSTS, ALSO MEASURED AND NOT HIDDEN. A column left out has no cell, so
        // TryGetCell answers false for it and a screen reader walking cells does not reach it -
        // exactly what row virtualization has always done to a row scrolled away (§54.3), now true
        // sideways as well. The row's spoken sentence is NOT affected, because Spoken builds it from
        // the column specs rather than from the cells that happen to exist.
